...
from src.SMAC.SimpleSMAC import create_SMAC
from types_ import *
...
def start_SMAC(problem_root_dir: str, target_instance_path: str, target_dimension: int,
    ...
    if candidate_init is None:
        ...
        known_x = list(target_x[target_indices]) + candidate_init
    np.random.seed(int(time.time() * 1000) % 10000)
    random.seed(int(time.time() * 1000) % 10000)
    x, my_problem = create_SMAC(problem=problem_instance, max_eval=max_eval, initial_solutions=known_x)
    return x, max(my_problem.step_history), my_problem.step_history, my_problem.eval_time, len(known_x)


def SMAC_task(result_dict, problem_root_dir: str, target_instance_path: str, gpu_index: int = 0,
    ...
    for initial_solution_num in initial_solution_nums:
        a = get_initial_solution_by_surrogate_mapping(problem_root_dir=problem_root_dir,
                                                      ...
                                                      randomly=False)
        x, y, step_history, eval_time, len_known = start_SMAC(
        ...
        #                                               gpu_index=gpu_index, eval_sample_num=eval_sample_num,
        #                                               randomly=True)
        # x, y, step_history, eval_time, len_known = start_hill_climbing(
        #     problem_root_dir=problem_root_dir,
        #     target_instance_path=target_instance_path,
        #     target_dimension=target_dimension,
        #     candidate_init=a,
        ...
        #     "len_known": len_known
        # }
    x, y, step_history, eval_time, len_known = start_SMAC(problem_root_dir=problem_root_dir,
                                                                           target_instance_path=target_instance_path,
                                                                           target_dimension=target_dimension,
                                                                           candidate_init=None,
                                                                           eval_sample_num=eval_sample_num,
...
def run_SMAC_multi(eval_sample_num: int = 64, max_eval: int = 6400,
                   problem_root_dir: str = "../data/problem_instance"):
    if not os.path.exists(Path("../SMAC_logs")):
        os.mkdir(Path("../SMAC_logs"))
    log_path = Path("../SMAC_logs", "mapSamplNum-{}maxEval-{}.pkl".format(eval_sample_num, max_eval))
    ...
    while len(compiler_task_list) > 0 or len(other_task_list) > 0:
        for element in compiler_task_list[:]:
            ...
            p = Process(target=SMAC_task, kwargs=task_args)
            p.start()
...
def output_multi_statistic(eval_sample_num: int = 64, max_eval: int = 6400, cut_eval=800,
    ...
    if not os.path.exists(Path("../SMAC_logs", "pics")):
        os.mkdir(Path("../SMAC_logs", "pics"))
    save_folder = Path("../SMAC_logs", "pics", "mapSamplNum-{}maxEval-{}".format(eval_sample_num, max_eval))
    ...
    for target_instance_path in os.listdir(Path(problem_root_dir, "valid")):
        ...
        results = [all_result["{}_{}_{}_{}".format(target_instance_path, eval_sample_num, max_eval, task_index)]
                   for task_index in range(30)]
        all_initial_data, all_step_data = {}, {}
...
if __name__ == '__main__':
    ...
    start = time.time()
    run_SMAC_multi(eval_sample_num=64, max_eval=800, problem_root_dir="../data/problem_instance")
