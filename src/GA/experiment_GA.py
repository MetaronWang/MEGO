...
from src.GA.simple_ga import SimpleBitGA
from types_ import *
...
def get_initial_solution_by_surrogate_mapping(problem_root_dir: str, target_instance_path: str, eval_sample_num=128,
                                              ...
                                              randomly: bool = False) -> Tuple[NpArray, List[NpArray]]:
    ...
    if not randomly:
        ...
        for dp in positive_correlation_dps:
            ...
            known_x += [solution for solution in solutions]
            know_x_quality += [dp.target_instance.evaluate(solution) for solution in solutions]
        sorted_index = np.argsort(know_x_quality)[::-1]
        for index in sorted_index:
            if len(initial_solution) >= initial_solution_num:
                break
            solution = known_x[index]
        ...
        for index in range(initial_solution_num):
            if len(initial_solution) >= initial_solution_num:
                break
            for result in generate_results:
                if len(initial_solution) >= initial_solution_num:
                    break
                solution = result[index]
                solution_str = "".join([str(bit) for bit in solution])
                if solution_str not in solution_strings:
                    solution_strings.add(solution_str)
                    initial_solution.append(solution)
    for dp in positive_correlation_dps:
        del dp
    gc.collect()
    return np.array(initial_solution), known_x


def start_ga(problem_root_dir: str, target_instance_path: str, target_dimension: int, known_x: List,
             eval_sample_num: int = 128, pop_size: int = 128, init_pop=None, max_iter: int = 5000,
             max_eval: int = 500000,
             elite: bool = False, elite_k=1):
    ...
    known_x = list(target_x[target_indices]) + known_x if init_pop is not None else []
    np.random.seed(int(time.time() * 1000) % 10000)
    random.seed(int(time.time() * 1000) % 10000)
    ga = SimpleBitGA(eval_func=problem_instance.evaluate, pop_size=pop_size, mut_prob=1 / target_dimension,
                     dim=target_dimension, max_iter=max_iter, max_eval=max_eval, elite=elite, elite_k=elite_k)

    if init_pop is not None:
        for x in known_x:
            # 先获取quality再添加，会记录eval_step
            ga.add_quality_record(x, ga.get_quality(x))

    ga.initial_population(initial_solutions=list(init_pop) if init_pop is not None else [])
    best_x, best_y = ga.run()
    pop_history: List[List[float]] = [population_quality for population_quality in ga.population_history]
    return best_x, best_y, pop_history, ga.eval_time, ga.step_history, len(known_x)


def ga_task(result_dict, problem_root_dir: str, target_instance_path: str, pop_size: int = 128, gpu_index: int = 0,
    ...
    for initial_solution_num in initial_solution_nums:
        a, known_x = get_initial_solution_by_surrogate_mapping(problem_root_dir=problem_root_dir,
                                                               ...
                                                               randomly=False)
        x, y, pop_history, eval_time, step_history, len_known = start_ga(problem_root_dir=problem_root_dir,
                                                                         target_instance_path=target_instance_path,
                                                                         target_dimension=target_dimension,
                                                                         pop_size=pop_size, init_pop=a,
                                                                         known_x=known_x, elite_k=elite_k,
                                                                         eval_sample_num=eval_sample_num,
                                                                         max_eval=max_eval, elite=True)
        if "compiler_args" in target_instance_path:
            print(target_instance_path, "Max Map Finished", datetime.datetime.now())
        result["max_map_{}_{}".format(pop_size, initial_solution_num)] = {
            "x": list(x),
            "y": y,
        ...
        # result["random_map_{}_{}".format(pop_size, initial_solution_num)] = {
        #     "x": list(x),
        #     "y": y,
        #     "pop_history": pop_history,
        #     "step_history": step_history,
    ...
    result_dict["{}_{}_{}_{}_{}".format(target_instance_path, eval_sample_num,
                                        pop_size, max_eval, task_index)] = deepcopy(result)
...
def run_ga_multi(eval_sample_num: int = 64, pop_size: int = 128, max_eval_rate: int = 10, elite_k: int = 1,
                 problem_root_dir: str = "../data/problem_instance"):
    if not os.path.exists(Path("../GA_logs")):
        os.mkdir(Path("../GA_logs"))
    log_path = Path("../GA_logs", "mapSamplNum-{}_popSize-{}_popEvalRate-{}_eliteK-{}"
                                  "_adjustInitial.pkl".format(eval_sample_num, pop_size, max_eval_rate, elite_k))
    ...
    for target_instance_path in os.listdir(Path(problem_root_dir, "valid")):
        for task_index in range(30):
            if "{}_{}_{}_{}_{}".format(target_instance_path, eval_sample_num, pop_size, task_args["max_eval"],
                                       task_index) in all_result.keys():
                print("{}_{}_{}_{}_{}".format(target_instance_path, eval_sample_num, pop_size, task_args["max_eval"],
                                              task_index))
                continue
            if "compiler_args" in target_instance_path:
    ...
    while len(compiler_task_list) > 0 or len(other_task_list) > 0:
        for element in compiler_task_list[:]:
            ...
            p = Process(target=ga_task, kwargs=task_args)
            p.start()
...
def print_distribution_population_curve(curve_data, save_folder, title, file_name):
    plt.clf()
    ...
    for initial_type in curve_data.keys():
        ...
        max_gen = min(len(gens) for gens in type_data)
        for task_data in type_data:
            for index in range(max_gen):
                if max(task_data[index]) > -1e8:
                    data["Generation"].append(index)
                    data["Max Fitness"].append(max(task_data[index]))
                    data["Initial Type"].append(initial_type)

    df = pds.DataFrame(data)
    g = sns.lineplot(df, x="Generation", y="Max Fitness", hue="Initial Type", style="Initial Type", markers=False,
...
def print_distribution_step_curve(curve_data, save_folder, title, file_name):
    plt.clf()
    plt.figure(figsize=[8, 6])
    data = {
        "Step": [],
        ...
        for task_data in type_data:
            for index in range(max_step):
                if task_data[index] > -1e8:
                    data["Step"].append(index)
                    data["Max Fitness"].append(task_data[index])
                    data["Initial Type"].append(initial_type)

    df = pds.DataFrame(data)
    g = sns.lineplot(df, x="Step", y="Max Fitness", hue="Initial Type", style="Initial Type", markers=False,
                     errorbar='sd', linewidth='1')
    # g.legend(prop={'family': 'Times New Roman', 'weight': 'bold'})
    ax = g.axes
    labels = ax.get_xticklabels() + ax.get_yticklabels()
...
def output_multi_statistic(eval_sample_num: int = 64, pop_size: int = 128, max_eval_rate: int = 10, cut_eval=800,
                           elite_k: int = 1, problem_root_dir: str = "../data/problem_instance"):
    print("\n\n\n------------------\n\n\n\n\n")
    print("eval_sample_num={}, pop_size={}, max_eval_rate={}".format(eval_sample_num, pop_size, max_eval_rate))
    all_result = pickle.load(
        open(Path("../GA_logs", "mapSamplNum-{}_popSize-{}_popEvalRate-{}_eliteK-{}"
                                "_adjustInitial.pkl".format(eval_sample_num,
                                                            pop_size, max_eval_rate,
                                                            elite_k)), "rb"))
    target_index = 0
    if not os.path.exists(Path("../GA_logs", "pics")):
        os.mkdir(Path("../GA_logs", "pics"))
    save_folder = Path("../GA_logs", "pics", "mapSamplNum-{}_popSize-{}_popEvalRate-{}_eliteK-{}"
                                             "_adjustInitial".format(
        eval_sample_num,
        pop_size, max_eval_rate,
        elite_k))
    ...
    for target_instance_path in os.listdir(Path(problem_root_dir, "valid")):
        ...
        results = [all_result["{}_{}_{}_{}_{}".format(target_instance_path, eval_sample_num,
                                                      pop_size, max_eval, task_index)]
                   for task_index in range(30)]
        all_population_data, all_step_data = {}, {}
        random_initial_data = [result["random_initial"]["y"] for result in results]
        all_population_data["random_initial"] = [result["random_initial"]["pop_history"] for result in results]
        ...
        for initial_type in ["max_map"]:
            for initial_solution_num in [2, 4]:
                map_data = [result["{}_{}_{}".format(initial_type, pop_size, initial_solution_num)]["y"] for result in
                            results]
                ...
                if ranksums(map_data, random_initial_data, alternative="greater")[1] < 0.05:
                    ...
                    print("→", end="\t")
                all_population_data["{}_{}".format(initial_type, initial_solution_num)] = [
                    result["{}_{}_{}".format(initial_type, pop_size, initial_solution_num)]["pop_history"] for result in
                    results]
                all_step_data["{}_{}".format(initial_type, initial_solution_num)] = [
                    hold_best(result["{}_{}_{}".format(initial_type, pop_size, initial_solution_num)]["step_history"][
                              :cut_eval])
                    for result in results]
        print("{:.2f}±{:.2f}".format(np.mean(random_initial_data), np.std(random_initial_data)))
        for initial_solution_num in [2, 4]:
            population_curve_data = {
                "random_initial": all_population_data["random_initial"],
                "max_map": all_population_data["max_map_{}".format(initial_solution_num)],
                # "random_map": all_population_data["random_map_{}".format(initial_solution_num)]
            }
            print_distribution_population_curve(curve_data=population_curve_data, save_folder=save_folder,
                                                file_name="{}_{}_pop".format(target_instance_path,
                                                                             initial_solution_num),
                                                title="{}, and the map solution num is {},\n"
                                                      "with pop_size={}, max_step={}, elite_k={},\n"
                                                      "while sample_num={} in decoder mapping".format(
                                                    target_instance_path, initial_solution_num, pop_size,
                                                    max_eval, elite_k, eval_sample_num
                                                ))
            step_curve_data = {
...
def output_multi_result(eval_sample_num: int = 64, pop_size: int = 128, max_eval_rate: int = 10, elite_k: int = 1,
                        problem_root_dir: str = "../data/problem_instance"):
    print("\n\n\n------------------\n\n\n\n\n")
    print("eval_sample_num={}, pop_size={}, max_eval_rate={}".format(eval_sample_num, pop_size, max_eval_rate))
    all_result = pickle.load(
    ...
    for target_instance_path in os.listdir(Path(problem_root_dir, "valid")):
        ...
        for task_index in range(30):
            ...
            result = all_result["{}_{}_{}_{}_{}".format(target_instance_path, eval_sample_num,
                                                        pop_size, max_eval, task_index)]
            for initial_type in ["max_map"]:
                for initial_solution_num in [4]:
                    print(result["{}_{}_{}".format(initial_type, pop_size, initial_solution_num)]["y"], end='\t')
...
if __name__ == '__main__':
    ...
    run_ga_multi(eval_sample_num=64, pop_size=32, max_eval_rate=25, elite_k=1,
                 problem_root_dir="../data/problem_instance")
    output_multi_result(eval_sample_num=64, pop_size=32, max_eval_rate=25, elite_k=1,
                        problem_root_dir="../data/problem_instance")
    output_multi_statistic(eval_sample_num=64, pop_size=32, max_eval_rate=25, elite_k=1,
                           problem_root_dir="../data/problem_instance")
